...
type Content = { id: string; type: ContentType; title: string; slug: string; excerpt?: string; body?: string; sectionKey?: string; published: boolean; media: Media[] }
type FormData = Omit<Content, 'id' | 'media'>

const API_URL = process.env.NEXT_PUBLIC_FCH_API_URL || 'http://localhost:3005/api'
const API_ORIGIN = API_URL.replace(/\/api\/?$/, '')
const initialForm: FormData = { type: 'section', title: '', slug: '', excerpt: '', body: '', sectionKey: '', published: true }
const labels: Record<ContentType, string> = { news: 'Notícias', post: 'Posts', section: 'Secções' }
...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<'conteudo' | 'eventos' | 'destaques'>('conteudo')
  ...
  const edit = (item: Content) => { setEditingId(item.id); setForm({ type: item.type, title: item.title, slug: item.slug, excerpt: item.excerpt || '', body: item.body || '', sectionKey: item.sectionKey || '', published: item.published }); setNotice('A editar: faça alterações e guarde.'); window.scrollTo({ top: 0, behavior: 'smooth' }) }
      ...
      <section className="relative grid w-full max-w-5xl overflow-hidden rounded-[2rem] bg-white shadow-[0_25px_80px_rgba(0,0,0,.45)] lg:grid-cols-[0.9fr_1.1fr] dark:bg-gray-900">
        <aside className="relative hidden overflow-hidden bg-gray-950 p-12 text-white lg:flex lg:flex-col lg:justify-between">
          ...
          <div className="relative">
            ...
            <p className="mt-7 max-w-xs text-sm leading-6 text-gray-300">Gestão editorial do site institucional, publicações, notícias e galerias de imagens.</p>
          </div>
