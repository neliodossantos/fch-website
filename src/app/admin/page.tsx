...
type Content = { id: string; type: ContentType; title: string; slug: string; excerpt?: string; body?: string; published: boolean; media: Media[] }
type FormData = Omit<Content, 'id' | 'media'>

const API_URL = process.env.NEXT_PUBLIC_FCH_API_URL || 'http://localhost:3005/api'
const API_ORIGIN = API_URL.replace(/\/api\/?$/, '')
const initialForm: FormData = { type: 'news', title: '', slug: '', excerpt: '', body: '', published: true }
...
export default function AdminPage() {
  ...
  const edit = (item: Content) => { setEditingId(item.id); setForm({ type: item.type, title: item.title, slug: item.slug, excerpt: item.excerpt || '', body: item.body || '', published: item.published }); setNotice('A editar: faça alterações e guarde.'); window.scrollTo({ top: 0, behavior: 'smooth' }) }
