...
export function VideoField({ token, value, onChange, label = 'Vídeo (link ou upload)' }: { token: string; value: string; onChange: (url: string) => void; label?: string }) {
  ...
  return (
    <div>
      <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-[#E4D9CC]">{label}</label>
      <input value={draft} onChange={event => setDraft(event.target.value)} onBlur={() => { if (draft !== value) onChange(draft) }} placeholder="https://youtube.com/watch?v=..." className="w-full rounded-lg border bg-white p-3 dark:bg-[#1f1a16]" />
      ...
      {error && <p className="mt-1 text-xs text-red-600 dark:text-[#fca5a5]">{error}</p>}
    </div>
  )
}
